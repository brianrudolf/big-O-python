...
def isupper(main_len=100, sub_len=10, num=10000):
    """Generate a larger string to search for a smaller string.
    Both of varying length, default is a length difference of 10x.

    Python doc description: Return True if there is at least one uppercase
        ASCII character in the sequence and no lowercase ASCII characters,
        False otherwise
    ...
    def time_isupper():
        ...
        timer = timeit.Timer(functools.partial(main_string.isupper))
    ...
    time_samples = [time_isupper() for i in range(num_random)]
...
if __name__ == "__main__":
    ...
    big_O = [isupper(main_len=x, sub_len=x // 10) for x in tqdm(n)]
