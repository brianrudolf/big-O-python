...
def islower(main_len=100, sub_len=10, num=10000):
    """Generate a larger string to search for a smaller string.
    Both of varying length, default is a length difference of 10x.

    Python doc description: Return True if there is at least one lowercase
        ASCII character in the sequence and no uppercase ASCII characters,
        False otherwise
    ...
    def time_islower():
        ...
        timer = timeit.Timer(functools.partial(main_string.islower))
    ...
    time_samples = [time_islower() for i in range(num_random)]
...
if __name__ == "__main__":
    ...
    big_O = [islower(main_len=x, sub_len=x // 10) for x in tqdm(n)]
